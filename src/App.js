...
function App() {
  ...
  const init = async () => {
    ...
    const signedBRTContractInstance = new Contract(
      BRTTokenAddress,
      BRTTokenAbi,
      signer
    );
  ...
  const onClickStake = async (e) => {
    ...
    if (stakeInput > userInfo.token_balance.toString())
      return alert("Insufficient available BRT");
  ...
  const onClickWithdraw = async (e) => {
    ...
    if (withdrawInput > 1000)
      return alert("Insufficient withdrawalable reward");
    const provider = new ethers.providers.Web3Provider(window.ethereum);
    const signer = provider.getSigner();
    const BRTContractInstance = new Contract(
      BRTTokenAddress,
  ...
  const onGetAddressBalance = async (e) => {
    ...
    setStakeDetails(utils.formatUnits(userDetails.stakeAmount.toString(), 18));
